...
public class MenuActions {
	...
	/**
	 * Costruttore della classe MenuActions che riceve come parametro un oggetto di tipo MenuBar.
	 * Questo metodo inizializza i campi dati menuBar e vl e richiama il metodo initialize().
	 * @param menuBar oggetto di tipo MenuBar
	 */
	...
	private void initialize() {
		...
		menuRecent.getItems().forEach(item -> {
			item.setOnAction(new EventHandler<ActionEvent>() {
				public void handle(ActionEvent e) {
					if (Main.getConnectionInstance().learnKnn(1, item.getText())) {
